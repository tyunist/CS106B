...
#include "lexicon.h"
#include "error.h"
#include "strlib.h"	// for ConvertToLowercase
...
class MatchPrefix {
  private:
	string p;
  public:
    MatchPrefix(const string &prefix) : p(toLowerCase(prefix)) {}
...
bool Lexicon::containsWord(string word) 
{
	...
	return otherWords.find(toLowerCase(word)) != otherWords.end();
}
...
void Lexicon::add(string word)
{
	if (!containsWord(word)) // don't add duplicate (set uniques, but need check dawg too)
		otherWords.insert(toLowerCase(word)); // we store words in all lowercase
}
